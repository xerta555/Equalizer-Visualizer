(function () {
  ...
  let currentFavoriteIndex = 0; // To see which favourite will be filled in next (0, 1, or 2)
  ...
  function updateCanvasSize(topContainer) {
	  ...
	  // Use maxBarHeight directly without modifying it
  ...
  let ctx = null; // Canvas context, defined in initializeVisualizer
  let smoothedAmplitudes = Array(barCount).fill(0); // Déplacer ici
  let lastFrameTime = 0; // Move here
  ...
  class Settings {
    ...
    FieldsContainer = () => {
      ...
      // Add a useState for each favourite to force re-rendering
      ...
      // Synchronise favourites values with the local state
      React.useEffect(() => {
          ...
          React.createElement(
            ...
            React.createElement('button', {
              ...
              onClick: () => {
                ...
                console.log(`[Equalizer Visualizer] Favourite colour 1 applied : ${barColor}`);
              }
            }, `rgba(${fav1})`),
            React.createElement('button', {
              style: {
              ...
              onClick: () => {
                ...
                this.setFieldValue('barColor', barColor);
                this.setRerender(Date.now());
                console.log(`[Equalizer Visualizer] Favourite colour 2 applied : ${barColor}`);
              }
            }, `rgba(${fav2})`),
            React.createElement('button', {
              style: {
              ...
              onClick: () => {
                ...
                this.setFieldValue('barColor', barColor);
                this.setRerender(Date.now());
                console.log(`[Equalizer Visualizer] Favourite colour 3 applied : ${barColor}`);
              }
            }, `rgba(${fav3})`)
          )
        ),
  ...
  settings.addInput('barColor', 'Bar Color (R,G,B,A)', barColor, (e) => {
    barColor = e.target.value;
    console.log(`[Equalizer Visualizer] Bar color updated to: ${barColor}`);
  }, 'text');
  settings.addInput('maxHeight', 'Maximum Bar Height (px)', maxBarHeight, (e) => {
  ...
  settings.addInput('testBars', 'Test Maximum Bar Height Display', null, () => {
    testMaxHeightBars();
  }, 'button');
  settings.addToggle('enabled', 'Enable/Disable Visualizer', isVisualizerEnabled, (e) => {
  ...
  let isTesting = false; // To avoid conflicts with normal animation

  function testMaxHeightBars() {
	  if (isTesting || !isVisualizerEnabled) return; // Do nothing if already in test or deactivated
	  isTesting = true;

	  const startTime = performance.now();
	  const animationDuration = 1500; // 1,5 second for the climb
	  const displayDuration = 3000; // 3 seconds of display at max height
	  const descentDuration = 1500; // 1,5 second for the descent
	  const originalFadeOutProgress = fadeOutProgress; // Saves the current state
	  ...
	  function animateBars(timestamp) {
		...
		if (progress < 1) {
		  ...
		  // Display at max height for 3 seconds
		  setTimeout(() => {
			  ...
			  if (performance.now() - maxDisplayStart < displayDuration) {
				...
				function animateDescent(timestamp) {
				  ...
				  if (descentProgress < 1) {
					...
					isTesting = false;
					fadeOutProgress = Spicetify.Player.isPlaying() ? 1 : originalFadeOutProgress; // Force to 1 if music is playing
					if (ctx) {
					  ctx.clearRect(0, 0, canvas.width, canvas.height); // Delete canvas
					}
					// Relaunch the main animation
					const topContainer = document.querySelector('.Root__top-container');
					if (topContainer && topContainer.contains(canvas)) {
					  requestAnimationFrame(animate); // Restarting normal animation
					}
				  }
				}
				requestAnimationFrame(animateDescent);
	  ...
	  fadeOutProgress = 1; // Forcing visibility during the test
	  requestAnimationFrame(animateBars);
	}
  ...
  function animate(timestamp) {
	  ...
	  if (!topContainer || !topContainer.contains(canvas) || !isVisualizerEnabled || isTesting) {
		if (isTesting) return; // Let the testMaxHeightBars function manage the display
		return;
	  }
	  ...
	  // Synchronise canvas height with maxBarHeight
	  if (canvas.height !== maxBarHeight) {
		canvas.height = maxBarHeight;
		canvas.style.height = `${maxBarHeight}px`;
	  }
  ...
  function initializeVisualizer() {
	  ...
	  // Force canvas height to be synchronised with maxBarHeight at start-up
	  canvas.height = maxBarHeight;
	  canvas.style.height = `${maxBarHeight}px`;
